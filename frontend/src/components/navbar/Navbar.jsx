...
const navLinkClassName = ({ isActive }) =>
  `relative py-1 text-sm font-medium transition-colors duration-150 ${
    isActive
      ? "text-brand after:absolute after:-bottom-[21px] after:left-0 after:h-0.5 after:w-full after:bg-brand"
  ...
  return (
    <header
      className={`sticky top-0 z-20 border-b transition-all duration-300 light:border-slate-200 light:bg-white/95 ${
        scrolled
          ? "border-brand/15 bg-[#090D0A]/90 shadow-lg shadow-black/40 backdrop-blur-xl light:shadow-slate-200/60"
          : "border-brand/10 bg-[#090D0A]/95 backdrop-blur"
      }`}
    >
      <div
          ...
          <div className="flex min-w-[180px] max-w-[280px] shrink-0 items-center gap-2.5 py-1 sm:min-w-[200px]">
            ...
            <div className="min-w-0">
              ...
              {/* Isotipo real de Smarticket (mismo patrón del wordmark de
                  arriba: cuadrado lima sólido + Ticket) a escala de firma —
                  nunca el wordmark completo acá. */}
              <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-400 light:text-slate-500">
                <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-md bg-brand">
      ...
      {mobileOpen && (
        <div
          id="mobile-nav-menu"
          className={`border-t border-white/5 bg-[#090D0A] px-4 py-4 light:border-slate-200 light:bg-white sm:px-6 ${
            brandOverride ? "2xl:hidden" : "lg:hidden"
          }`}
        >
          ...
          <nav aria-label="Navegación" className="flex flex-col gap-1">
            ...
            {isDeveloper && (
              <Link
                to="/developer"
                onClick={() => setMobileOpen(false)}
                className="flex items-center gap-1.5 rounded-lg px-3 py-2.5 text-sm font-medium text-brand transition-colors duration-150 hover:bg-white/5"
              >
                <LayoutDashboard className="h-4 w-4" />
                Panel developer
              </Link>
