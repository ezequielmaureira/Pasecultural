  ...
  return (
    <form onSubmit={handleSubmit} className={`relative ${className}`} data-search-scope={scope}>
      ...
      <input
        ...
        className="h-9 w-full rounded-lg border border-white/10 bg-white/5 pl-9 pr-3 text-sm text-gray-100 outline-none placeholder:text-slate-500 transition-colors duration-150 focus:border-brand focus:bg-white/10 focus:ring-2 focus:ring-brand/20"
      />
    </form>
  );
}
