...
export default function DatePicker({ value, onChange, placeholder = "Elegir fecha" }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute left-1/2 top-full z-20 mt-1 w-64 max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-lg border border-white/10 bg-[#111713] p-3 shadow-xl">
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-md text-xs transition-colors duration-150 disabled:cursor-not-allowed ${textClass} ${
                    isSelected ? "bg-brand-hover text-slate-950" : isToday ? "border border-brand/60" : ""
                  } ${!isPast && !isSelected ? "hover:bg-brand/20 hover:text-white" : ""}`}
                >
                  {date.getDate()}
                </button>
              );
