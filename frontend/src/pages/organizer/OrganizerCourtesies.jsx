            ...
            <div className="grid gap-4 sm:grid-cols-2">
                {ACTIONS.map(({ to, icon: Icon, title, description }) => (
                    <Link
                        key={to}
                        to={to}
                        className="group flex items-start gap-4 rounded-xl border border-white/10 bg-[#111713] p-6 transition-colors duration-150 hover:border-brand/40"
                    >
                        ...
                        <ChevronRight className="mt-2 h-4 w-4 shrink-0 text-slate-600 transition-colors duration-150 group-hover:text-brand" />
                    </Link>
                ))}
            </div>
        </div>
