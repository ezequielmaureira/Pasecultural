...
### Backend
Node.js con Express 5 (ESM puro, `type: module`). Prisma 6 como ORM sobre PostgreSQL (sin driver `pg` directo ni adapter externo). `@clerk/express` para autenticación de usuarios con cuenta. `jsonwebtoken` (JWT propio, sesiones de Scanner; el cifrado/secretos correspondientes usan `node:crypto` donde aplica). `resend` (email transaccional). `cloudinary` (imágenes). `pdfkit`/`qrcode` (PDF y QR generados del lado servidor). `multer` (upload de archivos). Sin TypeScript, sin linter configurado, sin framework de testing externo — usa el test runner nativo de Node (`node:test`).
