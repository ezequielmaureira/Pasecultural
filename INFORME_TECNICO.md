...
**Objetivo:** panorama general del organizador. **Estado: parcial — mezcla datos reales y datos permanentemente en cero.** **~50%.**
Implementado: banner de estado de la organización (pendiente/rechazada/suspendida/aprobada), tarjetas de "eventos activos/finalizados" y "entradas disponibles" con datos reales. Pendiente: "entradas vendidas", "recaudación total" y "recaudación del mes" muestran siempre 0 (no hay ningún endpoint de ventas agregadas conectado a este dashboard); los paneles "últimas ventas" y "últimos tickets escaneados" están siempre vacíos (mismo motivo) — ver sección 14.
...
| Ruta | Propósito | Estado |
|---|---|---|
| `/organizador` | Dashboard | Parcial (ventas/recaudación siempre en cero) |
...
**Servicios (`backend/src/services/`):** `auth.service.js`, `user.service.js`, `organization.service.js`, `event.service.js`, `functionCapacity.service.js`, `sale.service.js`, `saleRecoveryVerification.service.js`, `ticket.service.js`, `ticketAdmin.service.js`, `scanner.service.js`, `scannerInvitation.service.js`, `scannerLogin.service.js`, `eventScanner.service.js`, `scannerRead.service.js`, `media.service.js`, más el subárbol `email/` (`sendSaleConfirmationEmail.service.js`, `sendScannerVerificationCode.service.js`, `sendSaleRecoveryVerificationCode.service.js`, `ticketQrImages.js`, `ticketsPdf.js`, `formatDateAR.js`) y el subárbol `conversation/` (`EventCreationEngine.js`, `EventServicePort.js`, `steps/`, `inputHandlers/`, `errorMessages.js`).

**Controllers (`backend/src/controllers/`):** un archivo por dominio, espejo casi 1:1 de los services: `auth.controller.js`, `user.controller.js`, `organization.controller.js`, `event.controller.js`, `eventScanner.controller.js`, `sale.controller.js`, `ticket.controller.js`, `ticketAdmin.controller.js`, `scanner.controller.js`, `scannerRead.controller.js`, `scannerInvitation.controller.js`, `scannerAuth.controller.js`, `media.controller.js`. Todos siguen el mismo patrón: sólo validan `req`, llaman al service y devuelven la respuesta; la lógica de negocio vive exclusivamente en los services.

**Routes (`backend/src/routes/`):** `auth.routes.js`, `user.routes.js`, `organization.routes.js`, `media.routes.js`, `event.routes.js` (incluye las sub-rutas de scanners y de administración de entradas de un evento), `conversation.routes.js`, `sale.routes.js`, `ticket.routes.js`, `scanner.routes.js`, `scannerInvitation.routes.js`, `scannerAuth.routes.js`. Ver inventario completo de endpoints en la sección 10.
...
**Utilidades (`backend/src/utils/`):** entre otras, `getUserByClerkId.js`, `validateEmail.js`, `validateBuyerDocument.js` (normalización/validación de DNI, compartida por compra, recuperación y auditoría de entradas), `validateOrganization.js`, `organizationTrust.js` (`canPublishEvents`), `generateSlug.js`, `mediaParser.js` (detección de plataforma de un link pegado), `verificationCode.js` (generación/hash/comparación de códigos de 6 dígitos, compartida entre Scanner y recuperación de compra), `withTimeout.js` (compartido por los tres servicios de email), `htmlEscape.js`, `ticketNumber.js`.
...
Todas las entidades del `schema.prisma` (18 modelos, 15 enums), con propósito, relaciones y uso actual.
...
- **Sale** — una venta/orden de compra. Relaciona con `User` (comprador), `Event`, `EventFunction`, y 1 a N con `SaleItem`/`Ticket`. Incluye todo el seguimiento de envío del email de confirmación. Uso: activo, es la entidad central de la compra.
...
- **Ticket** — una entrada individual y escaneable, generada recién al confirmarse la venta. Relaciona con `Sale`, `Event`, `EventFunction`, `TicketType`, `User` (comprador y dueño), y 1 a N con `TicketQr`, `CheckIn`, `ScanAttempt`, `TicketAuditLog`. Uso: activo, es la entidad central del control de acceso.
...
**Enums:** `Role`, `UserStatus`, `OrganizationStatus`, `OrganizationType`, `EventStatus`, `EventVisibility`, `FunctionStatus`, `ConversationChannel`, `ConversationStatus`, `SaleStatus`, `PaymentMethod`, `EmailDeliveryStatus`, `TicketStatus`, `ScanResult`, `CheckInSource`, `TicketAuditAction`, `TicketAuditActorType`, `EventScannerStatus`.
...
**Organizador.** Banner de estado de la organización y tarjetas de eventos/entradas disponibles con datos reales; entradas vendidas, recaudación total y del mes, últimas ventas y últimos escaneos siempre en cero/vacío porque no hay ningún endpoint agregado de ventas conectado. Falta: un endpoint de métricas agregadas de ventas por organización, y conectar ese dashboard a él.
...
**Eventos:** `GET /api/events/scanner-events`, `POST /api/events`, `GET /api/events/mine`, `GET /api/events/:id`, `PATCH /api/events/:id`, `PUT /api/events/:id/schedule`, `PUT /api/events/:id/links`, `DELETE /api/events/:id`.
...
- **Dashboard del organizador**: eventos y entradas disponibles son reales; ventas, recaudación y actividad reciente están permanentemente en cero por falta de un endpoint de métricas de ventas.
...
2. **Endpoint de métricas de ventas agregadas por organización** — necesario para que el Dashboard del organizador deje de mostrar ceros permanentes.
3. **Conectar el Dashboard de Developer a datos reales** — hoy es 100% maqueta.
4. **Exportación real de entradas seleccionadas** (el botón ya existe en la UI, sólo falta la lógica).
5. **Reponer un control visible de filtro por estado** en la pantalla de administración de entradas (el backend y el estado ya lo soportan).
6. **Onboarding de datos de cobro del organizador** (ligado directamente a Mercado Pago).
7. **Notificaciones por email adicionales** (aprobación/rechazo de organización, venta nueva).
8. **Adaptador de WhatsApp** para el motor conversacional de creación de eventos (el modelo de datos ya lo contempla).
9. **Páginas legales reales** (Términos y Privacidad hoy enlazan a la home).
10. **Pantalla de perfil del usuario** (hoy es un stub).
...
- El Dashboard del Organizador depende de un contexto (`OrganizerDataContext`) cuyos campos `sales` y `recentScans` son arrays vacíos sin ningún setter expuesto — están ahí a propósito, a la espera de un backend de ventas agregadas que hoy no existe.
...
2. Construir el endpoint de métricas de ventas agregadas por organización y conectar el Dashboard del organizador.
...
**Avance aproximado del proyecto: ~70-75%.** El core funcional completo (catálogo de eventos, checkout, entradas con QR cifrado, control de acceso con garantías de concurrencia verificadas, recuperación de compra, administración de entradas con auditoría, paneles de Developer y Organizador para todo lo que no es venta) está implementado y, en la enorme mayoría de los casos, terminado de punta a punta backend+frontend. El hueco más grande, con diferencia, es el cobro real.

**Para una versión Beta** (uso real con usuarios externos pero controlado): falta, como mínimo, integrar Mercado Pago (sin esto no hay negocio real posible salvo ventas 100% manuales/gratuitas) y conectar el Dashboard del organizador a datos de ventas reales (hoy un organizador no tiene forma de ver cuánto vendió desde el panel). El resto del sistema ya está en condiciones de sostener una Beta acotada con pagos manuales/en efectivo mientras se construye la integración de pago.
